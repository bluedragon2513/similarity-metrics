...
def plotter(d1, d2, save_file, title=""):
    x = [1-d1[key] for key in d1.keys() if key in d2.keys()]
    y = [1-d2[key] for key in d1.keys() if key in d2.keys()]
...
if __name__ == "__main__":
    ...
    with open("src/experiment/kruskal/pancreas/celltype-scores-kruskal.pkl", "rb") as f:
    ...
    plotter(batch_scores, celltype_scores, "src/experiment/data/kruskal", title="Pancreas: batch vs. celltype (Scanorama)")